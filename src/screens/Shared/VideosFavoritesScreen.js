...
export function VideosFavoritesScreen(props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await video.getVideosFavoritesUser(accessToken, params.idUser);
        setVideos(response);
      } catch (error) {
        console.error(error);
      }
  ...
  return (
    <FlatList
      ...
      renderItem={({ item, index }) => (
        <VideoFeed item={item.video_data} index={index} indexShow={indexStart} style={{ height }} />
      )}
