...
export function Social(props) {
  ...
  const unfollowing = async () => {
    try {
      const response = await followController.getFollowing(accessToken, auth.user_id, idUser);
  ...
  return (
    <View style={styles.content}>
      {isFollowing === false ? (
        <Button title="Seguir" buttonStyle={styles.follow} containerStyle={styles.follow} onPress={follow} />
      ) : null}
